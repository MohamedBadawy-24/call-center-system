...
function SortableQuestionRow({ question, onRemove }) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      ...
      <span dir="auto" style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--primary)', padding: '0.1rem 0.4rem', background: 'var(--primary-low)', borderRadius: '4px' }}>
        Q
      </span>
      <div style={{ flex: 1, fontSize: '0.82rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--text-primary)' }}>
        {question.text || <span dir="auto" style={{ fontStyle: 'italic', color: 'var(--text-secondary)' }}>Untitled Question</span>}
      </div>
      <button dir="auto" 
        type="button" 
        onClick={onRemove}
    ...
    <div style={{ display: 'flex', gap: '1.5rem', height: 'calc(100vh - 140px)', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', paddingRight: '0.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: 'var(--primary)' }}>
          <Layers size={22} />
          <h2 dir="auto" style={{ fontSize: '1.2rem', fontWeight: 800, margin: 0 }}>Campaign Group Library</h2>
        </div>

        {(!surveyState.groups || surveyState.groups.length === 0) ? (
          <div className="glass-card" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
            <Layers size={48} style={{ opacity: 0.3, marginBottom: '1rem' }} />
            <p dir="auto" style={{ margin: 0, fontSize: '0.95rem', fontWeight: 600 }}>No question groups created yet.</p>
            <p dir="auto" style={{ margin: '0.5rem 0 0', fontSize: '0.82rem' }}>You can create groups here, or assign questions to groups inside the Advanced Display Logic panel of any question.</p>
          </div>
        ) : (
                ...
                <div key={grp._id} className="glass-card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                    {editingGroupId === grp._id ? (
                      <div style={{ display: 'flex', gap: '0.25rem', flex: 1 }}>
                        ...
                        />
                        <button dir="auto" type="button" className="btn-primary" onClick={() => handleSaveRename(grp._id)} style={{ padding: '0.25rem 0.5rem' }}>
                          <Check size={14} />
                        </button>
                        <button dir="auto" type="button" className="btn-secondary" onClick={() => setEditingGroupId(null)} style={{ padding: '0.25rem 0.5rem', color: 'var(--danger)' }}>
                          <X size={14} />
                        </button>
                      </div>
                    ) : (
                      <>
                        <h3 dir="auto" style={{ margin: 0, fontSize: '1rem', fontWeight: 800, color: 'var(--text-primary)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {grp.label}
                        </h3>
                        <div style={{ display: 'flex', gap: '0.25rem' }}>
                          <button dir="auto" type="button" className="btn-secondary" onClick={() => handleStartRename(grp._id, grp.label)} style={{ padding: '0.3rem', border: 'none' }} title="Rename Group">
                            <Edit2 size={13} />
                          </button>
                          <button dir="auto" type="button" className="btn-secondary" onClick={() => { if (confirm('Are you sure you want to delete this group? All questions inside will become standalone.')) deleteQuestionGroup(grp._id); }} style={{ padding: '0.3rem', border: 'none', color: 'var(--danger)' }} title="Delete Group">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </>
                  ...
                  {groupSections.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem', alignItems: 'center' }}>
                      <span dir="auto" style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>Sections:</span>
                      {groupSections.map((secName, sIdx) => (
                        <span dir="auto" key={sIdx} style={{ fontSize: '0.7rem', padding: '0.1rem 0.35rem', background: 'rgba(0,0,0,0.03)', border: '1px solid var(--border-color)', borderRadius: '4px', color: 'var(--text-secondary)' }}>
                          {secName}
                        </span>
                      ))}
                    </div>
      ...
      <div style={{ width: '280px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div className="glass-card" style={{ padding: '1.25rem' }}>
          <h3 dir="auto" style={{ margin: '0 0 1rem', fontSize: '0.95rem', fontWeight: 800, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          ...
          <form onSubmit={handleCreateGroup} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <label dir="auto" className="form-label" style={{ fontSize: '0.8rem', margin: 0 }}>Group Library Name</label>
              <input dir="auto"
                type="text"
                className="input-field"
            ...
            <button dir="auto" type="submit" className="btn-primary" style={{ padding: '0.55rem', fontSize: '0.85rem', width: '100%' }}>
              Add to Library
            </button>
          </form>
        </div>

...
async function getSurveyEligibilityState(user, surveyId, serialParam = null) {
  ...
  const serial = lastPrecall.payload?.serial_number != null ? String(lastPrecall.payload.serial_number) : "";
...
app.post("/agent/precall-complete", [auth, validatePrecallComplete], async (req, res) => {
  try {
    ...
    if (phoneInPayload && (!currentNumberDoc || currentNumberDoc.number !== phoneInPayload)) {
      // Phone number updated or manual entry — use provided serial or generate new
      ...
      // Update payload for consistency
      ...
      await doc.save();
        ...
        // Sync serial from PhoneNumber if missing in payload
    ...
    if (sid) {
      await PhoneNumber.findOneAndUpdate(
        ...
        { $set: { status: phoneStatus, calledAt: new Date(), outcomeReason: outcomeReason } },
        { sort: { assignedAt: -1 } }
      );
    }
...
app.get("/agent/search-serial/:serial", auth, async (req, res) => {
  try {
    ...
    // 1. Check Responses
    const response = await Response.findOne({ serialNumber: serial }).sort({ completedAt: -1 }).lean();
    if (response) {
    ...
    const precall = await PrecallCompletion.findOne({ serialNumber: serial }).sort({ completedAt: -1 }).lean();
    if (precall) {
      const phoneNumber = await PhoneNumber.findOne({ serialNumber: serial }).lean();
      return res.json({ 
    ...
    const phone = await PhoneNumber.findOne({ serialNumber: serial }).lean();
    if (phone) {
      return res.json({
...
app.get("/admin/export-advanced", staffAuth, async (req, res) => {
  try {
    ...
    const exportData = responses.map(r => {
      ...
      questions.forEach(q => {
        const answer = r.answers.find(a => a.questionId === q.id);
        row[q.text] = answer ? answer.value : "";
      });
      return row;
    });
...
app.get('/admin/survey/:id/numbers', [staffAuth, validateSurveyId], async (req, res) => {
  try {
    ...
    const stats = {
      total: await PhoneNumber.countDocuments({ surveyId: req.params.id }),
      pending: await PhoneNumber.countDocuments({ surveyId: req.params.id, status: 'pending' }),
      called: await PhoneNumber.countDocuments({ surveyId: req.params.id, status: 'called' }),
      qualified: await PhoneNumber.countDocuments({ surveyId: req.params.id, status: 'completed' }),
      disqualified: await PhoneNumber.countDocuments({ surveyId: req.params.id, status: 'disqualified' }),
      postponed: await PhoneNumber.countDocuments({ surveyId: req.params.id, status: 'postponed' }),
    };
...
app.get('/agent/next-number', auth, async (req, res) => {
  try {
    ...
    for (const s of targetSurveys) {
      ...
      number = await PhoneNumber.findOne({
        surveyId: s._id,
        agentId: user._id,
        status: 'pending'
      });
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    ...
    if (userId) {
      ...
      // Delay status change to handle brief reconnects or multi-tab usage
      const timer = setTimeout(async () => {
        try {
          // IMPORTANT: Check if the user is still connected via ANY other socket
          const isStillConnected = Array.from(activeSockets.values()).includes(userId);
          if (isStillConnected) {
            return; // Don't mark off-duty if another tab is open
          }

          const user = await User.findById(userId);
          // Only auto-off-duty if they are an agent or quality, and not already off-duty
          if (user && (user.role === 'agent' || user.role === 'quality') && user.currentStatus !== 'off-duty') {
            const now = new Date();
            ...
            console.log(`[System] User ${userId} marked off-duty due to full disconnect (60s timeout).`);
          }
        } catch (err) {
          console.error("Disconnect cleanup error:", err);
        }
      }, 60000); // 60 seconds (more lenient for flaky connections)
